...
package me.shunia.xsqst_helper.game.ui {
	...
	public class GameOverView extends Sprite implements ICtxCls {
	    ...
	    public function GameOverView() {
			_log = new LogPanel();
			addChild(_log);
			addEventListener(Event.ADDED_TO_STAGE, layout);
	    }
	
	    protected function layout(e:* = null):void {
			if (!parent) return;
	        ...
	        _settings.y = 0;
	        _settings.x = parent.width - _settings.width;
	
	        _log.x = 0;
	        _log.y = _user.height;
	        _log.width = parent.width - _settings.width;
	        _log.height = parent.height - _user.height;
	    }
		...
		public function onCtxInited():void {
			_e(_ctx).on("userSynced", function ():void {
				if (!_user) {
					_user = new UserStatusPanel(_ctx);
					_settings = new SettingsPanel(_ctx);
					addChild(_user);
					addChild(_settings);
				}
			});
