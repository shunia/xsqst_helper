...
package me.shunia.xsqst_helper.game {
	...
	public class GameService {
		...
		public function GameService(sid:String = null, uid:String = null) {
			_base.sid = sid;
			_base.uid = uid;
		}
        ...
        public function on(s:String, c:Function, ...args):void {
            var p:Params = R[s];
            if (p) {
                p.base(_base).args(args);
				GeneralService.on(p, function (data:String, ...args):void {
					var d:Object = null, 
					ps:Array = null;
					if (data.charAt(0) == "{") {    // json字符串
						d = JSON.parse(data);
						if (d && d.ret == 0) 
							ps = p.resHeader ? [d.data, args[0]] : [d.data];
						else if (d)
							ps = p.resHeader ? [d, args[0]] : [d];
...
class Params extends ReqParams {
	
	public function Params(j:String) {
		super(j);
		u = "http://s1.wangamemxwk.u77.com/service/main.ashx";
		header(WebCookie.from().toHeader());
	}
	...
	override public function o():ReqParams {
		_json = rpl(_json, "{uid}", _base.uid);
		_json = rpl(_json, "{sid}", _base.sid);
		return super.o();
	}
